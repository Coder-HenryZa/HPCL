...
stopwords = set([i.strip() for i in open(r'./data/stopwords.txt', 'r', encoding="utf-8").readlines()])
...
class UdGraphDataset(Dataset):
    def __init__(self, fold_x, treeDic, dataname, lower=2, upper=100000, droprate=0):
        ...
        self.treeDic = treeDic
        # self.data_path = data_path
        self.data_path = './data/' + dataname + '_Interaction'
        self.droprate = droprate

        self.seq_len = 35
        self.ws = pickle.load(
            open('./data/' + dataname + '_ws.pkl', 'rb'))

        statuses = pandas.read_csv('./data/' + dataname + '/' + dataname + '_Statuses.csv', sep='\t', encoding='utf-8')

        statuses_dict = statuses.set_index('twitter_id')['20_status'].to_dict()
        self.statuses_dict = statuses_dict

        sources = pandas.read_csv('./data/' + dataname + '/' + 'source_tweets.txt', sep='\t', encoding='utf-8',
                                  names=['twitter_id', 'status'])
    ...
    def __getitem__(self, index):
        ...
        return index, Data(x=torch.tensor(data['x'], dtype=torch.float32),
                           edge_index=torch.LongTensor(new_edgeindex),
                           y=torch.LongTensor([int(data['y'])]), root=torch.LongTensor(data['root']),
                           rootindex=torch.LongTensor([int(data['rootindex'])]),
                           tree_text_id=torch.LongTensor([int(id)]),
